...
## Nieuw: Android / Health Connect

Zie `ANDROID-INSTALLATIE.md` voor de APK-koppeling. Bij een bestaande v1-installatie: maak een databasebackup, vervang de publieke/private applicatiebestanden met behoud van `config/config.php`, en importeer `sql/migrations/002_health_connect.sql`. Bij een nieuwe installatie bevat `sql/install.sql` alles. Deze migratie maakt aparte importtabellen en een leesweergave; bestaande handmatige records worden niet gewijzigd.
...
| Excel | Applicatie |
|---|---|
| Gewicht | Gewicht in kg; weekgemiddelde en verschil met vorige week |
| Rode gewichtsmarkering: maaltijd niet zelfgemaakt (M1) | Apart veld ‘Een maaltijd niet zelfgemaakt?’; onbekend / nee / ja |
| Stappen | Dagwaarde, weektotaal en gemiddelde per ingevulde dag |
...
| Slaap: aantal uren op bed (M6) | ‘Tijd in bed’, opgeslagen als minuten (7u30 = 450); geen geschatte effectieve slaaptijd |
| Slaapkwaliteit | 1 = slecht tot 5 = uitgerust |
| Rusthartslag / bloeddruk | Optionele waarden; bloeddruk met boven- én onderdruk |
