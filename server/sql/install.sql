...
CREATE TABLE IF NOT EXISTS health_connect_daily (
 ...
 resting_hr SMALLINT UNSIGNED NULL, total_calories_kcal DECIMAL(10,2) NULL, body_fat_percentage DECIMAL(5,2) NULL,
 steps_sources TEXT NULL, weight_sources TEXT NULL, sleep_minutes_sources TEXT NULL, resting_hr_sources TEXT NULL,
 total_calories_kcal_sources TEXT NULL, body_fat_percentage_sources TEXT NULL,
 steps_synced_at DATETIME NULL, weight_synced_at DATETIME NULL, sleep_minutes_synced_at DATETIME NULL, resting_hr_synced_at DATETIME NULL,
 total_calories_kcal_synced_at DATETIME NULL, body_fat_percentage_synced_at DATETIME NULL,
 updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
 FOREIGN KEY(device_id) REFERENCES mobile_devices(id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
-- Health Connect is authoritative for automatically synchronized values; old manual values remain as fallback history.
CREATE OR REPLACE VIEW effective_daily_metrics AS
 ...
 COALESCE(h.sleep_minutes,d.sleep_minutes) AS sleep_minutes,d.sleep_quality,COALESCE(h.resting_hr,d.resting_hr) AS resting_hr,d.blood_pressure_sys,d.blood_pressure_dia,
 d.navel_cm,d.hip_cm,
