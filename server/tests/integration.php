...
$day=daily_input(['date'=>'2026-01-01','weight'=>'80']);upsert('daily_metrics',$day);$day['weight']=81;upsert('daily_metrics',$day);
verify((int)query('SELECT COUNT(*) FROM daily_metrics WHERE date=?',[$day['date']])->fetchColumn()===1,'daily idempotency');
verify((float)query('SELECT weight FROM daily_metrics WHERE date=?',[$day['date']])->fetchColumn()===81.0,'daily update');
