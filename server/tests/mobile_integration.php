...
try {
 mobile_transaction(function()use($id,$date){upsert('health_connect_daily',['date'=>$date,'device_id'=>$id,'steps'=>4000,'weight'=>82.4,'sleep_minutes'=>420,'resting_hr'=>58,'total_calories_kcal'=>2400,'body_fat_percentage'=>18.2]);});
 ...
 $r=query('SELECT * FROM effective_daily_metrics WHERE date=?',[$date])->fetch();ck((int)$r['steps']===4000&&(float)$r['weight']===82.4,'Health Connect values win when available');
 ck((int)$r['sleep_minutes']===420&&(int)$r['resting_hr']===58&&(float)$r['body_fat_percentage']===18.2,'extended Health Connect values available');
