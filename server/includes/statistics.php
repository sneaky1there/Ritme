...
function chart_series(array $rows,array $workouts,string $from,string $to): array {
    ...
    for($d=new DateTimeImmutable($from);$d<=new DateTimeImmutable($to);$d=$d->modify('+1 day')) {
      ...
      $days[]=['date'=>$date,'weight'=>isset($row['weight'])?(float)$row['weight']:null,'navel'=>isset($row['navel_cm'])?(float)$row['navel_cm']:null,'rolling'=>$weights?array_sum($weights)/count($weights):null];
    ...
    return ['days'=>$days,'weeks'=>array_values(array_map(fn($w)=>['week'=>$w['week'],'steps'=>total($w['rows'],'steps'),'sleep'=>($v=average($w['rows'],'sleep_minutes'))===null?null:$v/60,'workouts'=>$w['workouts'],'hc_sleep'=>($hc=average($w['rows'],'hc_sleep_minutes'))===null?null:$hc/60],$weeks))];
}
