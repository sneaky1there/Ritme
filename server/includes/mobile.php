...
function validate_mobile_snapshot(array $data): array {
    ...
    if(!is_array($metrics)||!count($metrics)||count($metrics)>3||array_values($metrics)!==$metrics) throw new InvalidArgumentException('Ongeldige meetvelden.');
    foreach($metrics as $metric) if(!is_string($metric)||!in_array($metric,['steps','weight','sleep_minutes'],true)) throw new InvalidArgumentException('Ongeldig meetveld.');
    ...
    foreach($rows as $index=>$row) {
        ...
        foreach($metrics as $metric) {
            ...
            $v=$row[$metric];$ranges=['steps'=>[0,200000,true],'weight'=>[20,500,false],'sleep_minutes'=>[0,1500,true]];
