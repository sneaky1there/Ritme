...
<div class="heading"><div><p class="eyebrow">WEEK <?=date('W')?> · <?=e((new DateTimeImmutable($week))->format('d-m'))?> — <?=date('d-m')?></p><h1>De week in beeld.</h1><p class="muted">Deze week tot nu toe. Gemiddelden gebruiken alleen ingevulde dagen.</p></div><span class="badge"><?=admin()||mobile_overview()?'Persoonlijk overzicht':'Coach · alleen lezen'?></span></div>
<div class="stats">
...
<article class="stat"><span>Gemiddeld lichaamsvet</span><strong><?=display_number($current['body_fat'])?><small> %</small></strong><p>Automatisch via Health Connect</p></article>
<article class="stat"><span>Totaal verbrand deze week</span><strong><?=display_number($current['calories_burned'],0)?><small> kcal</small></strong><p>Actief en basaal energieverbruik</p></article>
</div>
<details class="card"><summary><strong>Specificatie van deze week</strong> · bekijk de dagwaarden achter de totalen</summary><div class="table-wrap"><table><thead><tr><th>Dag</th><th>Gewicht</th><th>Lichaamsvet</th><th>Stappen</th><th>Tijd in bed</th><th>Slaapkwaliteit</th><th>Rusthartslag</th><th>Verbrand</th><th>Bloeddruk</th><th>Navel</th><th>Heup</th><th>Hevy-training</th></tr></thead><tbody>
<?php $currentByDate=array_column($filter($days,$week,'9999-12-31'),null,'date');$workoutsByDate=[];foreach($filter($workouts,$week,'9999-12-31') as $workout)$workoutsByDate[$workout['date']][]=$workout['workout_name'];for($i=0;$i<7;$i++):$day=(new DateTimeImmutable($week))->modify('+'.$i.' days');$date=$day->format('Y-m-d');$d=$currentByDate[$date]??[];?><tr><td><?=e(['zo','ma','di','wo','do','vr','za'][(int)$day->format('w')].' '.$day->format('d-m'))?></td><td><?=display_number($d['weight']??null)?> kg</td><td><?=display_number($d['body_fat_percentage']??null)?>%</td><td><?=display_number($d['steps']??null,0)?></td><td><?=sleep_label($d['sleep_minutes']??null)?></td><td><?=display_number($d['sleep_quality']??null,0)?></td><td><?=display_number($d['resting_hr']??null,0)?></td><td><?=display_number($d['total_calories_kcal']??null,0)?> kcal</td><td><?=isset($d['blood_pressure_sys'])?e($d['blood_pressure_sys'].' / '.$d['blood_pressure_dia']):'—'?></td><td><?=display_number($d['navel_cm']??null)?> cm</td><td><?=display_number($d['hip_cm']??null)?> cm</td><td><?=e(implode(', ',$workoutsByDate[$date]??[]))?:'—'?></td></tr><?php endfor;?></tbody></table></div><p class="muted">Lege dagen tellen niet mee in gemiddelden. Automatisch gesynchroniseerde Health Connect-waarden worden gebruikt zodra ze beschikbaar zijn.</p></details>
...
<p class="muted">Health Connect: <?= $mobileSync?'laatst bijgewerkt '.e($mobileSync).' UTC':'nog niet bijgewerkt' ?> · Gemiddelde slaapregistratie deze week: <?=sleep_label($hcSleep)?>.</p>
<section class="chart-grid" data-chart="<?=e(json_encode($chart,JSON_THROW_ON_ERROR))?>">
<?php foreach(['weight'=>'Gewicht & 7-daags gemiddelde','body_fat'=>'Lichaamsvet','resting_hr'=>'Rusthartslag','calories_burned'=>'Totaal verbrand per week','navel'=>'Navelomtrek','steps'=>'Stappen per week','sleep'=>'Tijd in bed per week','training'=>'Trainingen per week'] as $id=>$label): ?><article class="card chart-card <?= $id==='weight'?'wide':'' ?>"><h2><?=e($label)?></h2><div class="chart-box"><canvas id="chart-<?=e($id)?>" role="img" aria-label="<?=e($label)?>; waarden staan in de ruwe data"></canvas></div></article><?php endforeach; ?>
